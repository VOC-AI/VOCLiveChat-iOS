...
// 定义文件上传类型
typedef NS_ENUM(NSInteger, VocaiUploadFileType) {
    VocaiUploadFileTypePic = 0,
    VocaiUploadFileTypeFile = 1,
    VocaiUploadFileTypeVideo = 2,
} NS_SWIFT_NAME(VocaiUploadFileType);

@interface VocaiChatView()<WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIDocumentPickerDelegate, PDFDisplayViewDelegate, PHPickerViewControllerDelegate>
...
@property (nonatomic, assign) VocaiUploadFileType uploadFileType; ///  只用于上传失败的时候记录上传文件类型
...
- (void) initWebview {
    ...
    VocaiWebCacheManager *cacheManager = [VocaiWebCacheManager sharedManager];
    WKWebViewConfiguration *configuration = cacheManager.configuration;
...
- (void) loadPage {
    if (!self.webView || !self.vocaiChatParams) {
        return;
    }
...
-(void)showPickView {
    ...
    CustomOptionsView *optionsView = [[CustomOptionsView alloc] initWithFrame:CGRectMake(0, 0, viewWidth, viewHeight) language:self.language];
                    ...
                    [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
                        if (granted) {
                            ...
                            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                                // 后台任务...
                                dispatch_async(dispatch_get_main_queue(), ^{
                                    [self showNoPermissionToastOnView:self message:noPermissionHint];
                                });
                            });
                        }
                    }];
        ...
        if([result.itemProvider hasItemConformingToTypeIdentifier:(NSString *)kUTTypeMovie]) {
            [result.itemProvider loadItemForTypeIdentifier:(NSString *)kUTTypeMovie options:@{} completionHandler:^(__kindof id<NSSecureCoding>  _Nullable item, NSError * _Null_unspecified error) {
                ...
                if (videoURL) {
                    ...
                    [self uploadFile: videoData fileName: videoURL.lastPathComponent uploadFiledType: VocaiUploadFileTypeVideo];
                }
            }];
        }
        // 检查是否有图片数据
            ...
            [result.itemProvider loadObjectOfClass:[UIImage class] completionHandler:^(id<NSItemProviderReading>  _Nullable object, NSError * _Nullable error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if ([object isKindOfClass:[UIImage class]]) {
                        ...
                        [self uploadFile:imageData fileName:self.fileName uploadFiledType:VocaiUploadFileTypePic];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) {
        ...
        [self uploadFile:imageData fileName:fileName uploadFiledType:VocaiUploadFileTypePic];
...
- (void)uploadFile: (NSData *) fileData fileName:(NSString *)fileName uploadFiledType: (VocaiUploadFileType) uploadFileType {
    self.uploadFileType = uploadFileType;
    NSString *fileType = @"";
    switch (uploadFileType) {
        case VocaiUploadFileTypePic:
            fileType = @"image/jpeg";
            break;
        case VocaiUploadFileTypeFile:
            fileType = @"application/pdf";
            break;
        case VocaiUploadFileTypeVideo:
...
- (void) handleUploadFileError: (VocaiUploadFileType)uploadFileType errorMessage: (NSString*)message  {
    dispatch_async(dispatch_get_main_queue(), ^{
 ...
 */
- (void)refreshWebViewWithUploadFiledType: (VocaiUploadFileType)uploadFileType toUrl: (NSString*)url {
    if (uploadFileType == VocaiUploadFileTypePic) { // Picture
        dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (uploadFileType == VocaiUploadFileTypeFile) { // File
        dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (uploadFileType == VocaiUploadFileTypeVideo) { // Video
        dispatch_async(dispatch_get_main_queue(), ^{
            NSDictionary *urlDict = @{@"url": url,
                                      @"reserveId": self.randomNumber};
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls {
    if (urls.count > 0) {
        ...
        if (data) {
            ...
            [self uploadFile:data fileName: self.fileName uploadFiledType:VocaiUploadFileTypeFile];
        } else {
...
- (void)displayPdfViewWithUrl:(NSString *)url {
    ...
    self.pdfDisplayView = [[PDFDisplayView alloc] initWithFrame:self.bounds pdfURL: pdfURL];
    self.pdfDisplayView.delegate = self;
    [self addSubview:self.pdfDisplayView];
}
